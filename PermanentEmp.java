...
public class PermanentEmp extends Employee {
	...
	public void calculate_balance_leaves() {
		...
		if (avail_leave(take_paid_leave, 'P')) // Check if the employee can avail paid leave
												
		{
			paid_leave = paid_leave - take_paid_leave; 
		...
		if (avail_leave(take_sick_leave, 'S'))// Check if the employee can avail SICK LEAVES
												
		{
			sick_leave = sick_leave - take_sick_leave;// Update the pending SICK LEAVES
														
			take_sick_leave = 0;// Reset the taken SICK LEAVES thus NO DEDUCTION in salary
		...
		if (avail_leave(take_casual_leave, 'C'))// Check if the employee can avail CASUAL LEAVES
												
		{
			casual_leave = casual_leave - take_casual_leave;// Update the pending CASUAL LEAVES
			take_casual_leave = 0;
		} else
			casual_leave = 0;
	}
	...
	public boolean avail_leave(int no_of_leaves, char type_of_leave) {
		if (type_of_leave == 'P') {
			if (paid_leave - take_paid_leave >= 0) // Check if leave can be availed
			...
			if (casual_leave - take_casual_leave >= 0)// Check if leave can be availed
